...
def plot_peakredshift(data_pre, data,  plotname, path):
    try:
        ...
        print("mean delta on peak redshift for data_zs_lim with model is:", data_eval)
        print("rms delta on peak redshift for data_zs_lim with model is:", data_eval_2)

        out_file = open(str(path)+ "/PeakPhotoz"+str(plotname)+".txt", "w")
        ...
        data["max_pis"]=data["max_pis_y"]
        data["sigs"]=data["sigs_y"]
        ###p>0.8
        print(data.columns)
        data_8=data[data["max_pis"]>= 0.8]
        delta = (data_8["PhotoZ"] - data_8["zs"]) / (1 + data_8["zs"])
        ...
        out_file = open(str(path)+"/PeakPhotoz_p80" + str(plotname) + ".txt", "w")
        out_file.write("mean delta:%s\n" % data_eval)
        out_file.write("rms delta:%s\n" % data_eval_2)
        ...
        def range_eval(data_range, name_range):
            ...
            out_file = open(str(path)+ "/PeakPhotoz" + str(plotname) + str(name_range)+".txt", "w")
            out_file.write("mean delta:%s\n" % data_eval)
            out_file.write("rms delta:%s\n" % data_eval_2)
        ...
        def range_eval_cl(data_range, name_range, cl_):
            ...
            out_file = open(str(path)+ "/PeakPhotoz" + str(plotname) + str(cl_) + str(name_range) + ".txt", "w")
            out_file.write("mean delta:%s\n" % data_eval)
            out_file.write("rms delta:%s\n" % data_eval_2)
            ...
            # Plot the histogram.
            plt.hist(delta, bins=700, density=True)#, alpha=0.6, color='b')

            # Plot the PDF.
            xmin, xmax = plt.xlim()
            x_hist = np.linspace(xmin, xmax, 700)
            p_hist = norm.pdf(x_hist, mu_hist, std_hist)
            plt.plot(x_hist, p_hist, 'k', linewidth=2)
            plt.title(r"Fit results: $\mu$ = {:0.02f},  $\sigma$ = {:0.02f}".format(mu_hist, std_hist))
            #
            plt.xlabel(r"$\Delta Z$")
            plt.savefig(str(path)+ "/Peakredshift_deltaHist_" + str(plotname) + str(cl_) + str(name_range) + ".png")



            data_range_8=data_range[data_range["max_pis"]>=0.8]
            delta = (data_range_8["PhotoZ"] - data_range_8["zs"]) / (1 + data_range_8["zs"])

            data_eval = np.mean(delta)
            data_eval_2 = np.sqrt(np.mean(np.power(delta, 2)))

            out_file = open(str(path)+ "/PeakPhotoz_p80_" + str(plotname) + str(cl_) + str(name_range) + ".txt", "w")
            out_file.write("mean delta:%s\n" % data_eval)
            out_file.write("rms delta:%s\n" % data_eval_2)



        data["max_pis"]=data["max_pis_x"]
        data["sigs"]=data["sigs_x"]
        print("DATAs columns", data.columns)
        x = data["zs"]
        photoz = data["PhotoZ"]
        # deltaZ_Z_=df_uncertanity["deltaZ_Z"]
        ...
        probability = data["max_pis"]
        sigmas = data["sigs"]
        res= data["PhotoZ"] - data["zs"]

        print("min photoz+1", np.min(photoz+1))
        print("min zph_lim", np.min(zph_lim))
        print("min prob", np.min(probability))
        print("min sigma", np.min(sigmas))


        plt.figure(figsize=(10, 12))
        fig, (ax1, ax2, ax3, ax4) = plt.subplots(nrows=4, sharex=True, gridspec_kw={
            # 'width_ratios': [1, 1],
        ...
        plt.savefig(str(path)+"/Peakredshift_hist_"+str(plotname)+".png")
        ...
        print("DATAs columns", data.columns)
        x = data_8["zs"]
        photoz = data_8["PhotoZ"]
        # deltaZ_Z_=df_uncertanity["deltaZ_Z"]

        zph_lim = 0.01 * (1 + data_8["PhotoZ"])

        deltaZ_A_ = data_8["deltaZ_A"]
        # errorz= np.abs(x- photoz)/x+1
        probability = data_8["max_pis"]
        sigmas = data_8["sigs"]
        res = data_8["PhotoZ"] - data_8["zs"]
        ...
        ax1.plot((0, 7), (0, 7), linestyle='--', color='grey', linewidth=0.1)
        h1 = ax1.hist2d(x, photoz, bins=700, norm=colors.LogNorm(),
                        cmap=my_cmap)  # , cmin=1, vmin=0)
        ax1.set_ylabel(r"$z_{photo}$")
        ax1.set_ylim(-0.2, 7)
        ...
        # ax1.title.set_text("Objects:" + str(len(data_8)) )#+ "frac:{:0.2f}".format(frac))

        plt.savefig(str(path)+ "/Peakredshift_p80_hist_" + str(plotname) + ".png")
        ...
        for cl_ in class_list:
            ...
            if cl_ == "GALAXY":
                ...
                res=  data_lim_class_plot_zoom["PhotoZ"] - data_lim_class_plot_zoom["zs"]
                errorz = np.abs(x - photoz) / (x + 1)
                ...
                plt.savefig(str(path) +"/Peakredshift_hist_Zoom_" + str(plotname) + str(cl_) + ".png")
                ...
                if cl_ == "GALAXY":
                    data_lim_class_plot = data[data["class"] == cl_]
                    data_lim_class_plot_8=data_lim_class_plot[data_lim_class_plot["max_pis"]>0.8]
                    data_lim_class_plot_zoom = data_lim_class_plot_8[data_lim_class_plot_8["zs"] < 0.3]
                    data_lim_class_plot_zoom = data_lim_class_plot_zoom[data_lim_class_plot_zoom["PhotoZ"] < 0.3]
                    x = data_lim_class_plot_zoom["zs"]
                    photoz = data_lim_class_plot_zoom["PhotoZ"]
                    ...
                    plt.savefig(str(path) + "/Peakredshift_p80_hist_Zoom_" + str(plotname) + str(cl_) + ".png")
...
def outliers_peakredshift(data_pre, data, plotname,path ):
    try:
        delta = (data["PhotoZ"] - data["zs"]) / (1 + data["zs"])
        #
        # data_eval = np.mean(delta)
        ...
        df_misszs_.to_pickle(str(path) + "/df_misszs_" + str(plotname) + ".pkl")



        out_file = open(str(path) + "/PeakPhotoz_outliers_frac" + str(plotname) + ".txt", "w")
...
def ZoomGal_outliers_peakredshift(data_pre, data, plotname, path):
    print("data type", type(data))
    print(len(data))
    print(data.columns)
    print(data.tail())
    ...
    try:
        ...
        df_misszs_ = pd.merge(df_misszs, data, on="index")
        print("df_misszs_", df_misszs_.tail())
        df_misszs_.to_pickle(str(path) + "/df_misszs_ZOOM_GALAXY_" + str(plotname) + ".pkl")



        out_file = open(str(path) + "/PeakPhotoz_outliers_frac_ZOOM_GALAXY_" + str(plotname) + ".txt", "w")
